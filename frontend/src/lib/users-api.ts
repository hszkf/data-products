...
import type { User, UserRole, Team } from './auth-context';
...
export async function resetUserPassword(id: number, newPassword: string): Promise<void> {
  const response = await authFetch(`${API_BASE_URL}/users/${id}/reset-password`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
export async function toggleUserActive(id: number): Promise<User> {
  const response = await authFetch(`${API_BASE_URL}/users/${id}/toggle-active`, {
    method: 'POST',
  });
